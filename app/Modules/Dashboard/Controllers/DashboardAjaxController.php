	...
	public function ajaxSaveTransactionData(Request $Request) {
		if($Request->isMethod('POST') && !empty($Request->order_id)) {
			...
			if($DashboardOrderData->total_price < $Request->payment_amount * 100) {
				return Response::json(['status' => true, 'errors' => true, 'message' => [0 => 'შეყვანილი ღირებულება აღემატება შეკვეთის ღირებულებას']]);
			}
			...
			dd([$DashboardOrderTransactionDataCount->sum('amount'), $DashboardOrderTransactionDataCount->sum('amount') + ($Request->payment_amount * 100)]);
